...
Статус: **принятая спецификация baseline CORE**.
...
`developer_mode` нужен разработческому Site, где создаётся Standard metadata. В этом практикуме он намеренно задаётся Site-local только для `rental.localhost`, чтобы финальный acceptance-site проверял установку App без developer mode. Это **архитектурное решение учебного стенда**, а не утверждение, что официальная документация Frappe предписывает именно Site-local область настройки.
...
# 17. Контроль принятого baseline CORE
...
Если новое требование меняет один из ответов, пересматривается соответствующая часть спецификации, а не весь CORE автоматически.