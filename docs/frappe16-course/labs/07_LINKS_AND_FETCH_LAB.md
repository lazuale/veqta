...
В существующем Request очисти `Responsible`.
...
Заверши ввод значения в Link-поле.

Ожидаемый результат:

```text
Frappe отклоняет not-a-real-user@example.test
→ значение не принимается как корректный User
→ Document не получает несуществующую Link-ссылку
```

При проверке ссылки Framework ищет целевой `User`; отсутствующий Document не является допустимым значением `Link → User`.

Не создавай User с таким именем ради прохождения ошибки.
