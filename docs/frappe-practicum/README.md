...
Практикум **не является частью VEQTA и не использует её предметную модель**.

То, что файлы курса сейчас лежат в репозитории `lazuale/veqta`, — только место хранения. Учебные app, site, данные, примеры и сценарии должны оставаться полностью нейтральными, чтобы каталог `docs/frappe-practicum/` можно было перенести в отдельный репозиторий без переработки содержания.

В практикуме не используются:

- DocType VEQTA;
- продуктовые решения VEQTA;
- её dev-site;
- её app;
- её Git-история;
- названия и бизнес-сценарии VEQTA.
