...
- графики `Активные по статусу` и `Новые работы` отображаются без ошибок;
- `Активные по статусу` показывает русские подписи групп.

Отдельного графика по `priority` в русской конфигурации нет: штатный Dashboard Chart v16 вывел бы сырые `Low / Medium / High`.
