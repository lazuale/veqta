...
| комментарии и обсуждение | Form Timeline / Comments | использовать штатно |
...
| интеграции | REST API, Webhook, hooks | не создавать собственный integration framework |
...
Таким образом, наличие Kanban, Gantt, уведомлений или повторяемости не является основанием добавлять новые движки или служебные DocType в Work Management.
...
`Work Membership` нужен для рабочих представлений, фильтра выбора исполнителя, аналитики состава команды и, если конкретная установка этого требует, проверки назначения исполнителя в свою рабочую зону.
...
`Basis Document` используется как `Work Source`, но не является обязательным источником для всех организаций. В другой предметной области источником может быть Ticket, Alert, Contract, Nonconformity или другой DocType.
...
Frappe `Track Changes` и `Version` остаются базовой технической историей. Для продвинутой операционной аналитики может использоваться отдельная узкая история значимых событий Work Item:
...
`Work Event` фиксирует только события с бизнес-смыслом: начало, изменение состояния, назначение, передача, переоткрытие, завершение. Он не является общим event bus и не заменяет `Version`.

Его задача — дать стабильную основу для анализа времени в состояниях, handover, reassignments, reopen и других характеристик потока работы, которые неудобно и ненадёжно вычислять из технического diff.
...
`Basis Document` и `Work Project` не требуют добавлять специальные поля в Work Item: связь уже выражается через `sources` и `references`.
...
Frappe Framework
│
├── permissions / Workflow / Notifications
├── Desk / Views / Reports / Workspace
├── Auto Repeat / File / Comments
├── REST / Webhooks / hooks
│
└── Work Management
    │
