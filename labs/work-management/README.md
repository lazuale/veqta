...
Если Site уже был настроен по предыдущей конфигурации и содержит `Work Item.status = Open / Waiting / Closed / Cancelled`, перед переходом на русские значения выполните [миграцию статусов](russian-status-migration.md). Простая замена Options без переноса существующих документов не допускается.
