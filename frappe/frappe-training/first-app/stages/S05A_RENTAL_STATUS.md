...
Это **новая ответственность** — хранить текущее предметное состояние Rental.

Для неё достаточно обычного поля:
...
Workflow
Is Submittable
docstatus как предметный статус
...
Проверьте, что Rental уже существует как Standard DocType, принадлежащий App:
...
После S04 рабочее дерево должно быть чистым.
...
В дальнейшем `Planned` будет неблокирующим состоянием для проверки занятости Equipment.
...
Отвечает за системный транзакционный жизненный цикл:
...
Rental пока не является Submittable Document.
...
```text
предметный status ≠ Workflow ≠ docstatus
```
...
Если сервер разработки не запущен:
...
Мы изменяем **Standard DocType собственного App**, поэтому его метаданные должны измениться в исходниках `rental_training`.
...
# 7. Проверить метаданные в исходниках App
...
Поскольку `rental.json` уже существовал и отслеживался Git, изменение должно быть видно обычным diff:

```bash
git -C apps/rental_training status --short
```

Ожидайте изменение существующего файла метаданных Rental.
...
→ изменение метаданных App
→ Git видит изменение
```

---

# 8. Не считать, что Default автоматически переписал старые Documents
...
Не пишите patch ради одной учебной записи Site разработки.

Позже отдельно разберём разницу между:

```text
новыми метаданными и схемой
и
преобразованием существующих данных
```
...
Поэтому сохранение двух конфликтующих Active Rentals сейчас означает только то, что соответствующее правило ещё не реализовано.
...
Если позднее появится реальное требование согласования или ограничения переходов, тогда будет проверяться Workflow или серверная предметная логика по её семантике.
...
Предметное состояние и транзакционный жизненный цикл отвечают на разные вопросы.
...
→ предметное состояние и системный транзакционный жизненный цикл различны.
```

---

# 17. Проверка перед следующими этапами
...
- `Default = Planned`;
- поле mandatory;
- поле доступно для стандартной фильтрации и List-сценария;
- `Workflow` для Rental не создан;
- `Is Submittable = OFF`;
- ученик показал рядом `status` и `docstatus` через серверный API или console;
- Rental с `status = Returned` всё ещё имеет `docstatus = 0`;
- изменение метаданных находится в Git App;
- commit сделан;
- ученик может объяснить различие `status / Workflow / docstatus`.

---

# 18. Когда не переходить дальше
...
- обязательное поле создано как локальный Custom Field и не попало в метаданные App;
...
Теперь модель знает **что происходит с Rental**. Следующие этапы добавят обычный сценарий Desk, серверные инварианты и модель permissions.