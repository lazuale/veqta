...
Почему именно через штатный редактор DocType: Frappe рассматривает DocType как основной объект метаданных модели. Нам не нужно вручную создавать SQL-таблицу, а затем отдельно описывать ORM и форму.
...
При включённом developer mode Standard DocType сохраняется в исходники App. Именно это состояние потом можно перенести на чистый Site через Git и migrate.

Если включить `Custom?`, получится DocType, локальный для конкретного Site. Это другая ответственность и другой способ владения конфигурацией.

На S02 нужен именно:

```text
Standard DocType, принадлежащий App
```
...
Официальное руководство Frappe отдельно предупреждает, что для генерации файлов DocType `Custom?` должен быть выключен.
...
Название может измениться, поэтому мы не используем его как системный идентификатор Document.
...
В практикуме пока нет бизнес-требования:
...
У любого обычного DocType Frappe есть системное поле `name`. Это первичный ключ и стабильный идентификатор Document.
...
Следовательно он не может выполнять роль обязательного идентификатора.

Официальное руководство Frappe по naming определяет `name` как уникальный ID Document и поддерживает `Expression` как штатную схему именования.
...
Это первая важная практическая демонстрация архитектуры на основе метаданных:
...
Официальная документация описывает DocType как базовый элемент приложения и указывает, что после его создания Frappe предоставляет стандартные List и Form views.
...
```text
__init__.py
equipment.json    метаданные DocType
equipment.py      Python Controller
equipment.js      клиентский Controller
test_equipment.py заготовка тестов
```

Точный набор служебных файлов определяется текущим Frappe, поэтому критерием является не буквальное количество файлов, а наличие метаданных Standard DocType и предусмотренных мест для Controller и тестов внутри App.

### Главное

Мы **не создавали вручную** `equipment.json`, `equipment.py`, `equipment.js` и заготовку тестов.

Их создал штатный механизм Frappe для Standard DocType.

Официальное руководство показывает тот же принцип: JSON описывает DocType, Python-файл является Controller, JS-файл — клиентским Controller, а test-файл создаётся как заготовка тестов.
...
Формат JSON может содержать больше технических полей метаданных, чем мы вводили в форме. Это нормально.

Нужно понять связь:

```text
редактор DocType в Desk
        ↓ сохраняет
...
То есть UI здесь не создаёт «магическую настройку отдельно от кода». Для Standard DocType метаданные являются частью исходного состояния App.
...
На чистом прохождении ожидаются значения из модели практикума.
...
В модели есть проверка отображения Title в Link Fields.
...
Для проверки S02 прямой доступ к MariaDB не требуется.

Не выполняйте ручные `INSERT`, `UPDATE`, `ALTER TABLE` или другие изменения `tabEquipment` через MariaDB.

Причина простая: приложение должно работать через модель Frappe, а не обходить жизненный цикл Document и метаданные вручную.
...
Добавьте созданные Frappe исходники в индекс Git:
...
Новые файлы Equipment должны принадлежать репозиторию `rental_training`.
...
Теперь метаданные модели хранятся в исходниках App и могут участвовать в дальнейшей проверке чистой установки.
...
```text
Нужен стабильный идентификатор
```
...
Frappe создал `equipment.py`, но на S02 у нас нет бизнес-требования, для которого нужно собственное поведение Controller.

Поэтому Python Controller сейчас может быть почти пустым.

Мы не добавляем искусственную функцию только для того, чтобы «попробовать Python».

Код появится тогда, когда возникнет ответственность, которую метаданные сами не гарантируют — например серверные инварианты Rental.
...
Почему плохо: человекочитаемое название может исправляться и не обязано быть подходящим стабильным идентификатором.
...
Почему плохо: обязательная модель учебного App останется локальной для Site вместо исходников App.
...
Почему плохо: мы обходим штатный механизм Frappe — DocType и Document.

Правильное решение:

```text
метаданные DocType → Frappe создаёт схему
```
...
В Git присутствуют файлы Standard DocType, созданные Frappe.

---

# 23. Проверка перед S03

Перед переходом дальше ученик должен без подсказки объяснить:
...
9. Где в репозитории лежат метаданные Equipment?
...
# 24. Когда не переходить к S03

Сначала исправьте причину, если:
...
- `equipment_name` или `serial_number` необоснованно сделали системным идентификатором;
- naming не соответствует выбранной Expression-схеме;
- title и `name` воспринимаются как одно и то же;
- обязательные метаданные App отсутствуют в Git;
