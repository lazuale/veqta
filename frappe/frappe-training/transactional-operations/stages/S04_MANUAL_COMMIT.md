...
Ручной commit также завершил транзакцию, в которой удерживались row locks. Это ещё одна причина не использовать его как «страховку»: он преждевременно меняет не только durability, но и границу конкурентного критического участка.
...
`git restore` возвращает рабочую реализацию с `Rental FOR UPDATE`, V03 locking reads и без ручного commit.
...
Он **меняет границу бизнес-операции и преждевременно освобождает её row locks**.
...
Rental FOR UPDATE восстановлен
Git App чист
