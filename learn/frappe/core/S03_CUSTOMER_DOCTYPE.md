...
- [`../CORE_STAGE_SPECIFICATION.md`](../CORE_STAGE_SPECIFICATION.md) — точная модель Customer;
- [`../PRACTICUM_ROADMAP.md`](../PRACTICUM_ROADMAP.md) — архитектурное место этапа.
...
Проверьте, что `Equipment` уже существует как App-owned DocType:
...
```text
имеет ли Customer самостоятельную identity?
→ да
...
- `Custom? = OFF` — модель принадлежит App и должна попасть в source;
- Customer — самостоятельный объект, поэтому не Child Table;
- клиентов много, поэтому не Single;
- у Customer нет транзакционного submit/cancel lifecycle.
...
Не делайте Phone или Email системной identity.

Причины простые:

- у человека может не быть заполненного телефона/email;
- контакт может измениться;
- два человека могут использовать общий контакт;
- бизнес-требования CORE не говорят, что эти значения глобально уникальны.

---

# 6. Настроить naming

На вкладке/секции Naming выберите штатный Expression naming:
...
Для CORE достаточно одного фиксированного штатного Expression.
...
```text
что видит пользователь ≠ identity связанного Document
```
...
Для них просто нет требований текущего CORE.

---

# 9. Сохранить Customer и проверить source
...
Ожидайте App-owned metadata и стандартные controller/test locations, например:
...
Точный boilerplate может меняться, поэтому важен контракт:

```text
Customer metadata находится в source rental_training
```
...
Не нужно вручную переписывать generated JSON после сохранения только ради того, чтобы «сделать всё кодом».
...
Если sequence уже использовалась во время экспериментов, фактический номер может быть другим. Проверяется формат `CUST-#####`, а не искусственный сброс счётчика.
...
```text
изменяемое business display value
не должно случайно быть identity Document
```
...
identity начинает зависеть от display value
```
...
Email — контактное свойство, а не гарантированная неизменная identity Customer.

## Ошибка 3. Email проверяется своей regex

Если нужна обычная email-валидация, Frappe уже предоставляет `Data + Options = Email`.

Собственная regex без дополнительного требования дублирует framework responsibility.

## Ошибка 4. Customer вообще не создаётся как Document

Копирование имени/телефона/email в каждый Rental создаёт денормализованную копию там, где предметная модель требует живой самостоятельной сущности.
...
почему Email validation не требует своей regex
...
- Customer создан как `Custom` site-only DocType;
...
Они имеют собственную identity, Form/List и App-owned metadata, но пока не связаны друг с другом.
